...
public class BrowseBookmarkFeedFragment extends ListFragment
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    mAdapter =
        new SimpleCursorAdapter(
            getActivity(),
            R.layout.bookmark_feed_view,
            null,
            new String[] {Bookmark.Description, Bookmark.Tags},
            new int[] {R.id.bookmark_feed_description, R.id.bookmark_feed_tags},
            0);
  ...
  public void onResume() {
    ...
    if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
      ...
      if (tagname != null && tagname != "") {
        getActivity()
            .setTitle(getString(R.string.browse_user_bookmarks_tagged_title, feed, tagname));
      } else {
  ...
  public Loader<Cursor> onCreateLoader(int id, Bundle args) {
    if (Intent.ACTION_SEARCH.equals(intent.getAction())) {
      String query = intent.getStringExtra(SearchManager.QUERY);
      return new LoaderDrone(
          getActivity(), username, query, feed, AccountHelper.getAccount(username, getActivity()));
    } else {
      return new LoaderDrone(
          getActivity(),
          username,
  ...
  public static class LoaderDrone extends AsyncTaskLoader<Cursor> {
    ...
    public Cursor loadInBackground() {
      ...
      if (feed.equals("global")) feed = "";

      try {
        if (feed.equals("network")) {
          String token =
              AccountManager.get(getContext()).getUserData(account, Constants.PREFS_SECRET_TOKEN);
  ...
  public void onAttach(Activity activity) {
    ...
    try {
      ...
      throw new ClassCastException(
          activity.toString() + " must implement OnBookmarkSelectedListener");
    }
  }
}
