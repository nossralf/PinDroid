...
public class AddBookmarkFragment extends Fragment implements PindroidFragment {
  ...
  public void onActivityCreated(Bundle savedInstanceState) {
    ...
    mEditDescription =
        ((FloatLabel) getView().findViewById(R.id.add_edit_description)).getEditText();
    ...
    mEditTags =
        (MultiAutoCompleteTextView)
            ((FloatLabel) getView().findViewById(R.id.add_edit_tags)).getEditText();
    ...
    if (username != null) {
      CursorAdapter autoCompleteAdapter =
          new TagAutoCompleteCursorAdapter(
              getActivity(),
              R.layout.autocomplete_view,
              null,
              new String[] {Tag.Name, Tag.Count},
              new int[] {R.id.autocomplete_name, R.id.autocomplete_count},
              0);

      autoCompleteAdapter.setFilterQueryProvider(
          new FilterQueryProvider() {
            public Cursor runQuery(CharSequence constraint) {
              return TagManager.GetTagsAsCursor(
                  (constraint != null ? constraint.toString() : ""),
                  username,
                  Tag.Count + " DESC, " + Tag.Name + " ASC",
                  getActivity());
            }
          });
    ...
    mEditUrl.setOnFocusChangeListener(
  ...
  private void save() {
    ...
    if (titleTask != null) titleTask.cancel(true);

    if (tagTask != null) tagTask.cancel(true);
    ...
    for (String s : tags) {
      if (!s.equals("") && !s.equals(" ")) tagstring += (s + " ");
    }
  ...
  TagSpan.OnTagClickListener tagOnClickListener =
      new TagSpan.OnTagClickListener() {
