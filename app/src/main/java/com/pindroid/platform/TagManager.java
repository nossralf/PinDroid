...
public class TagManager {
  ...
  public static CursorLoader GetTags(String account, String sortorder, Context context) {
    ...
    return new CursorLoader(
        context, Tag.CONTENT_URI, projection, selection, selectionargs, sortorder);
  }

  public static Cursor GetTagsAsCursor(
      String query, String account, String sortorder, Context context) {
    final String[] projection = new String[] {Tag._ID, Tag.Name, Tag.Count};
    String selection = null;
    String[] selectionargs = new String[] {account, query + "%"};

    if (query != null) {
      selection = Tag.Account + "=?" + " AND " + Tag.Name + " LIKE ?";
    } else {
    ...
    return context
        .getContentResolver()
        .query(Tag.CONTENT_URI, projection, selection, selectionargs, sortorder);
  }
  ...
  public static void UpsertTag(Tag tag, String account, Context context) {
    ...
    final Cursor c =
        context
            .getContentResolver()
            .query(Tag.CONTENT_URI, projection, selection, selectionargs, null);
