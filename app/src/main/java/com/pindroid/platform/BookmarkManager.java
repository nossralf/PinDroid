...
public class BookmarkManager {
  ...
  public static CursorLoader GetBookmarks(
      String username,
      String tagname,
      boolean unread,
      boolean untagged,
    ...
    String[] selectionargs =
        new String[] {username, "% " + tagname + " %", "% " + tagname, tagname + " %", tagname};
    ...
    return new CursorLoader(
        context, Bookmark.CONTENT_URI, projection, selection, selectionargs, sortorder);
  }
  ...
  public static ArrayList<Bookmark> GetLocalBookmarks(String username, Context context) {
    ...
    selectionargs = new String[] {username};
    selection =
        Bookmark.Account + "=? AND " + Bookmark.Synced + "<>1 AND " + Bookmark.Deleted + "=0";

    Uri bookmarks = Bookmark.CONTENT_URI;

    Cursor c =
        context.getContentResolver().query(bookmarks, projection, selection, selectionargs, null);
      ...
      do {
        ...
        Bookmark b =
            new Bookmark(
                ...
                c.getString(tagsColumn),
                c.getString(hashColumn),
                c.getString(metaColumn),
                0,
                c.getInt(readColumn) == 0 ? false : true,
  ...
  public static ArrayList<Bookmark> GetDeletedBookmarks(String username, Context context) {
    ...
    selectionargs = new String[] {username};
    selection =
        Bookmark.Account + "=? AND " + Bookmark.Synced + "=0 AND " + Bookmark.Deleted + "=1";
  ...
  public static Bookmark GetByUrl(String url, String username, Context context)
    ...
    String selection =
        Bookmark.Url + "=? AND " + Bookmark.Account + "=? AND " + Bookmark.Deleted + "=0";
    final String[] selectionargs = new String[] {url, username};

    Cursor c =
        context
            .getContentResolver()
            .query(Bookmark.CONTENT_URI, projection, selection, selectionargs, null);
  ...
  public static Bookmark GetByHash(String hash, String username, Context context)
      throws ContentNotFoundException {
    final String[] projection =
        new String[] {
          Bookmark._ID,
    ...
    String selection =
        Bookmark.Hash + "=? AND " + Bookmark.Account + "=? AND " + Bookmark.Deleted + "=0";
  ...
  public static void UpdateBookmark(Bookmark bookmark, String account, Context context) {
    ...
    if (bookmark.getTime() > 0) values.put(Bookmark.Time, bookmark.getTime());
    ...
    Uri uri =
        Bookmark.CONTENT_URI.buildUpon().appendPath(Integer.toString(bookmark.getId())).build();
  ...
  public static void TruncateBookmarks(
      ArrayList<String> accounts, Context context, boolean inverse) {
    ...
    if (accounts.size() > 0) selection += " AND " + Bookmark.Synced + "=1";
    else selection += Bookmark.Synced + "=1";

    context.getContentResolver().delete(Bookmark.CONTENT_URI, selection, null);
  }

  public static CursorLoader SearchBookmarks(
      String query, String tagname, boolean unread, String username, Context context) {
    final String[] projection =
        new String[] {
          Bookmark._ID,
          Bookmark.Url,
  ...
  public static int GetAllBookmarksCount(String username, Context context) {
    if (username == null || username.equals("")) return 0;
