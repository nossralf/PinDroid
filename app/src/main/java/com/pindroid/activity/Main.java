...
public class Main extends FragmentBaseActivity
  ...
  public void onCreate(Bundle savedInstanceState) {
    ...
    Drawable drawable =
        VectorDrawableCompat.create(getResources(), R.drawable.drawer_spinner, getTheme());
  ...
  private void _initMenu() {
    ...
    for (int res = 0; res < myMenuItems.length; res++) {
      ...
      int id_title =
          getResources().getIdentifier(myMenuItems[res], "string", this.getPackageName());
      int id_icon =
          getResources().getIdentifier(myMenuItemsIcon[res], "drawable", this.getPackageName());
    ...
    for (int res = 0; res < feedMenuItems.length; res++) {
      ...
      int id_title =
          getResources().getIdentifier(feedMenuItems[res], "string", this.getPackageName());
      int id_icon =
          getResources().getIdentifier(feedsMenuItemsIcon[res], "drawable", this.getPackageName());
    ...
    if (tagData != null) {
      ...
      while (tagData.moveToNext()) {
        ...
        if (tags.size() == 0 || tags.contains(tagData.getString(1))) {
          NsMenuItemModel mItem =
              new NsMenuItemModel(
                  tagData.getString(1), R.drawable.ic_label_gray_24dp, false, tagData.getInt(2));
          mAdapter.addItem(mItem);
          res++;
        }
      }
  ...
  private void processIntent(Intent intent) {
    ...
    String lastPath =
        (intent.getData() != null && intent.getData().getLastPathSegment() != null)
            ? intent.getData().getLastPathSegment()
            : "";
    String intentUsername =
        (intent.getData() != null && intent.getData().getUserInfo() != null)
  ...
  protected void changeAccount() {
    ...
    if (accounts.size() > 0) {
      ...
      accountSpinnerView.setOnClickListener(
          new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    if (unreadItem != null) {
      ...
      ((NsMenuAdapter) ((HeaderViewListAdapter) mDrawerList.getAdapter()).getWrappedAdapter())
  ...
  private void toggleAccountSpinner(Boolean animate) {
    accountSpinnerButton.setImageResource(
        accountSpinnerOpen
    ...
    if (animate) {
      ...
      if (accountSpinnerOpen) {
        animation =
            new ResizeAnimation(
                accountList,
                accountList.getChildCount()
                    * (int) getResources().getDimension(R.dimen.account_list_height),
                0,
                true,
      ...
      accountList.getLayoutParams().height =
          accountSpinnerOpen
              ? 0
              : accountList.getChildCount()
                  * (int) getResources().getDimension(R.dimen.account_list_height);
    ...
    if (BookmarkViewType.EDIT.equals(viewType)) {
      ...
      if (isTwoPane()) {
        ...
        if (fragmentManager.findFragmentByTag("right") instanceof ViewBookmarkFragment) {
          ViewBookmarkFragment viewFrag =
              (ViewBookmarkFragment) fragmentManager.findFragmentByTag("right");
        ...
        if (fragmentManager.findFragmentByTag("left") instanceof ViewBookmarkFragment) {
          ViewBookmarkFragment viewFrag =
              (ViewBookmarkFragment) fragmentManager.findFragmentByTag("left");
          viewFrag.setBookmark(b, viewType);
          viewFrag.refresh();
  ...
  SharedPreferences.OnSharedPreferenceChangeListener prefListner =
      new SharedPreferences.OnSharedPreferenceChangeListener() {
        public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
          if (key.equals(
              getApplicationContext().getResources().getString(R.string.pref_drawertags_key))) {
