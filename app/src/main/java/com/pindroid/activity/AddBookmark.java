...
public class AddBookmark extends AppCompatActivity
    implements AddBookmarkFragment.OnBookmarkSaveListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    AddBookmarkFragment frag =
        (AddBookmarkFragment)
            getSupportFragmentManager().findFragmentById(R.id.addbookmark_fragment);
    frag.setUsername(getIntent().getStringExtra("username"));
    frag.loadBookmark(
        (BookmarkContent.Bookmark) getIntent().getParcelableExtra("bookmark"),
        (BookmarkContent.Bookmark) getIntent().getParcelableExtra("oldBookmark"));
  }
