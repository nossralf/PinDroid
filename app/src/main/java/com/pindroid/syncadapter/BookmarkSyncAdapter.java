...
public class BookmarkSyncAdapter extends AbstractThreadedSyncAdapter {
  ...
  public void onPerformSync(
    ...
    boolean manual =
        extras.containsKey(ContentResolver.SYNC_EXTRAS_IGNORE_BACKOFF)
    ...
    try {
      if (upload) {
        ...
        if (manual) Log.d(TAG, "Beginning Manual Download Sync");
  ...
  private void DeleteBookmarks(Account account, SyncResult syncResult)
    ...
    final ArrayList<Bookmark> bookmarks =
        BookmarkManager.GetDeletedBookmarks(account.name, mContext);
  ...
  private ArrayList<Bookmark> getBookmarkList()
    ...
    do {
      morePages =
          results.addAll(
              PinboardApi.getAllBookmarks(null, page++ * pageSize, pageSize, mAccount, mContext));
    } while (morePages);

    return results;
  }

  private void checkSecretToken(Account account)
      throws AuthenticationException, IOException, TooManyRequestsException, ParseException,
          PinboardException {
  ...
  /**
   * This helper function fetches the last known high-water-mark we received from the server - or 0
   * if we've never synced.
   *
