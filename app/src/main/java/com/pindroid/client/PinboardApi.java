...
public class PinboardApi {
   ...
   * @return An Update object containing the timestamp and the number of new bookmarks in the inbox.
  ...
  public static Boolean deleteBookmark(Bookmark bookmark, Account account, Context context)
    ...
    if (result.getCode().equalsIgnoreCase("done")
        || result.getCode().equalsIgnoreCase("item not found")) {
      return true;
    } else {
   ...
   *
   * @param hashes A list of bookmark hashes to be retrieved. The hashes are MD5 hashes of the URL
   *     of the bookmark.
   * @param account The account being synced.
   * @param context The current application context.
  ...
  public static ArrayList<Bookmark> getBookmark(
      ArrayList<String> hashes, Account account, Context context)
      throws IOException, AuthenticationException, TooManyRequestsException, PinboardException {
  ...
  public static ArrayList<Bookmark> getAllBookmarks(
      String tagName, Account account, Context context)
      throws IOException, AuthenticationException, TooManyRequestsException, PinboardException {
   ...
   */
  public static ArrayList<Bookmark> getAllBookmarks(
      String tagName, int start, int count, Account account, Context context)
      throws IOException, AuthenticationException, TooManyRequestsException, PinboardException {
    ArrayList<Bookmark> bookmarkList = new ArrayList<Bookmark>();
   ...
   * @throws PinboardException
   */
  private static InputStream PinboardApiCall(
      String url, TreeMap<String, String> params, Account account, Context context)
      throws IOException, AuthenticationException, TooManyRequestsException, PinboardException {
    ...
    String authtoken =
        "00000000000000000000"; // need to provide a sane default value, since a token that is too
                                // short causes a 500 error instead of 401

    try {
      String tempAuthtoken = am.blockingGetAuthToken(account, Constants.AUTHTOKEN_TYPE, true);
      if (tempAuthtoken != null) authtoken = tempAuthtoken;
    } catch (Exception e) {
      e.printStackTrace();
