...
public class BookmarkContentProvider extends ContentProvider {
  ...
  public static class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqlDb) {
      ...
      sqlDb.execSQL(
          "CREATE INDEX " + TAG_TABLE_NAME + "_ACCOUNT ON " + TAG_TABLE_NAME + " " + "(ACCOUNT)");
      ...
      sqlDb.execSQL(
          "CREATE INDEX " + NOTE_TABLE_NAME + "_ACCOUNT ON " + NOTE_TABLE_NAME + " " + "(ACCOUNT)");
    }
  ...
  private Uri insertBookmark(Uri uri, ContentValues values) {
    ...
    if (rowId > 0) {
      Uri rowUri =
          ContentUris.appendId(BookmarkContent.Bookmark.CONTENT_URI.buildUpon(), rowId).build();
  ...
  public Cursor query(
    ...
    switch (sURIMatcher.match(uri)) {
      ...
      case UnreadCount:
        ...
        return rdb.rawQuery(
            "select count(*) as Count, ACCOUNT as Account from "
                + BOOKMARK_TABLE_NAME
                + " where "
                + Bookmark.ToRead
                + "=1 group by "
  ...
  private Cursor getBookmark(
      Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
    SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
    SQLiteDatabase rdb = dbHelper.getReadableDatabase();
    qb.setTables(BOOKMARK_TABLE_NAME);
    qb.appendWhere(
        Bookmark._ID + "=" + uri.getPathSegments().get(Bookmark.BOOKMARK_ID_PATH_POSITION));
  ...
  private Cursor getSearchSuggestions(String query, boolean accountSpecific) {
    ...
    Map<String, SearchSuggestionContent> tagSuggestions =
        new TreeMap<String, SearchSuggestionContent>();
    Map<String, SearchSuggestionContent> bookmarkSuggestions =
        new TreeMap<String, SearchSuggestionContent>();
    Map<String, SearchSuggestionContent> noteSuggestions =
        new TreeMap<String, SearchSuggestionContent>();
  ...
  private Map<String, SearchSuggestionContent> getBookmarkSearchSuggestions(
      String query, boolean accountSpecific) {
    Log.d(TAG, "getBookmarkSearchSuggestions query=" + query);

    String[] bookmarks = query.split(" ");

    Map<String, SearchSuggestionContent> suggestions =
        new TreeMap<String, SearchSuggestionContent>();
    ...
    for (String s : bookmarks) {
      bookmarkList.add("(" + Bookmark.Description + " LIKE ? OR " + Bookmark.Notes + " LIKE ?)");
    ...
    String[] projection =
        new String[] {BaseColumns._ID, Bookmark.Description, Bookmark.Url, Bookmark.Account};

    Cursor c =
        getBookmarks(
            Bookmark.CONTENT_URI,
            projection,
            selection,
            selectionlist.toArray(new String[] {}),
            null,
            SuggestionLimit);
    ...
    if (c.moveToFirst()) {
      ...
      do {
        ...
        suggestions.put(
            title + "_bookmark_" + account,
            new SearchSuggestionContent(
                title, line2, url, R.drawable.ic_bookmark_blue_24dp, data.toString(), action));
  ...
  private Map<String, SearchSuggestionContent> getTagSearchSuggestions(
      String query, boolean accountSpecific) {
    ...
    if (c.moveToFirst()) {
      ...
      do {
        ...
        if (!accountSpecific && accountCount > 1) tagCount = account;
        ...
        suggestions.put(
            ...
            new SearchSuggestionContent(
                name,
                tagCount,
                R.drawable.ic_label_gray_24dp,
                data.build().toString(),
                Constants.ACTION_SEARCH_SUGGESTION_VIEW));
  ...
  private Map<String, SearchSuggestionContent> getNoteSearchSuggestions(
      String query, boolean accountSpecific) {
    ...
    for (String s : notes) {
      noteList.add("(" + Note.Title + " LIKE ? OR " + Note.Text + " LIKE ?)");
      selectionlist.add("%" + s + "%");
      selectionlist.add("%" + s + "%");
    ...
    if (c.moveToFirst()) {
      ...
      do {
        ...
        if (!accountSpecific && accountCount > 1) text = account;

        suggestions.put(
            title + "_note_" + account,
            new SearchSuggestionContent(
                title,
                text,
                R.drawable.ic_note_brown_24dp,
                data.toString(),
                Constants.ACTION_SEARCH_SUGGESTION_VIEW));
  ...
  public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
    ...
    boolean syncOnly =
        values.size() == 1
            && values.containsKey(Bookmark.Synced)
            && values.getAsInteger(Bookmark.Synced) == 1;
  ...
  private static UriMatcher buildUriMatcher() {
    ...
    matcher.addURI(
        AUTHORITY, "global/" + SearchManager.SUGGEST_URI_PATH_QUERY, GlobalSearchSuggest);
    matcher.addURI(
        AUTHORITY, "global/" + SearchManager.SUGGEST_URI_PATH_QUERY + "/*", GlobalSearchSuggest);
    ...
    matcher.addURI(
        AUTHORITY, "tag/" + SearchManager.SUGGEST_URI_PATH_QUERY + "/*", TagSearchSuggest);
    matcher.addURI(
        AUTHORITY, "bookmark/" + SearchManager.SUGGEST_URI_PATH_QUERY, BookmarkSearchSuggest);
    matcher.addURI(
        AUTHORITY,
        "bookmark/" + SearchManager.SUGGEST_URI_PATH_QUERY + "/*",
        BookmarkSearchSuggest);
    matcher.addURI(AUTHORITY, "note/" + SearchManager.SUGGEST_URI_PATH_QUERY, NoteSearchSuggest);
    matcher.addURI(
        AUTHORITY, "note/" + SearchManager.SUGGEST_URI_PATH_QUERY + "/*", NoteSearchSuggest);
    return matcher;
  }
}
